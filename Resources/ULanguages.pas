...
interface
uses
  System.Generics.Collections, UConsts;
...
//Arabic
  ...
  Answer('إجابة').AskMenu('اسأل ' + CPluginName).DockableMenu(CPluginName + ' قابل للإرساء').SettingMenu('إعدادات ' + CPluginName).BaseUrl('عنوان URL الأساسي').ApiKey('مفتاح الوصول').
...
//Chinese
  ...
  AskMenu('询问 ' + CPluginName).DockableMenu(CPluginName + ' 可停靠').SettingMenu(CPluginName + ' 设置').BaseUrl('基本网址').ApiKey('访问密钥').Model('模型').MaxToken('最大令牌').
...
//English
  ...
  Question('Question').Answer('Answer').AskMenu('Ask ' + CPluginName).DockableMenu('Dockable ' + CPluginName).SettingMenu(CPluginName + ' Setting').BaseUrl('Base URL:').
...
//French
  ...
  AutoClipBoard('Copie automatique dans le presse-papiers').Question('Question').Answer('Répondre').AskMenu('Demander à ' + CPluginName).DockableMenu(CPluginName + ' ancrable').
  SettingMenu('Paramètre ' + CPluginName).BaseUrl('URL de base').ApiKey('Clef d''accès').Model('Modèle').MaxToken('Max-Token').Temperature('Température').
...
//Germany
  ...
  AutoClipBoard('Automatisches Kopieren in die Zwischenablage').Question('Frage').Answer('Antworten').AskMenu('Fragen Sie ' + CPluginName).DockableMenu('Andockbares ' + CPluginName).
  SettingMenu(CPluginName + '-Einstellung').BaseUrl('Basis-URL').ApiKey('Zugangsschlüssel').Model('Modell').MaxToken('Max Token').Temperature('Temperatur').
...
//Hindi
  ...
  Question('सवाल').Answer('उत्तर').AskMenu(CPluginName + ' से पूछें').DockableMenu(CPluginName + ' डॉक करने योग्य').SettingMenu(CPluginName + ' सेटिंग').BaseUrl('बेस यूआरएल').
...
//Italian
  ...
  Question('Domanda').Answer('Risposta').AskMenu('Chiedi a ' + CPluginName).DockableMenu(CPluginName + ' agganciabile').SettingMenu('Impostazioni ' + CPluginName).BaseUrl('URL di base').
...
//Japanese
  ...
  Question('質問').Answer('答え').AskMenu(CPluginName + 'に質問する').DockableMenu('ドッキング可能 ' + CPluginName).SettingMenu(CPluginName + '設定').BaseUrl('ベース URL').
...
//Kurdish(Kurmanci)
  ...
  Question('Soru').Answer('Cevap').AskMenu(CPluginName + '''ye Sor').DockableMenu('Takılabilir ' + CPluginName).SettingMenu(CPluginName + ' Ayarı').BaseUrl('Temel URL').
...
//Persian
  ...
  Question('سوال').Answer('پاسخ').AskMenu('از ' + CPluginName + ' بپرسید').DockableMenu(CPluginName + ' قابل اتصال').
   SettingMenu('تنظیمات ' + CPluginName).BaseUrl('URL پایه').ApiKey('کلید دسترسی').Model('مدل').MaxToken('حداکثر توکن').Temperature('درجه حرارت').Identifier('شاخص').
...
//Portuguese
  ...
  AutoClipBoard('Cópia automática para a área de transferência').Question('Pergunta').Answer('Responder').AskMenu('Pergunte ao ' + CPluginName).DockableMenu(CPluginName + ' acoplável').
   SettingMenu('Configuração do ' + CPluginName).BaseUrl('URL base').ApiKey('Chave de acesso').Model('Modelo').MaxToken('Max-Token').Temperature('Temperatura').
...
//Russian
  ...
  Question('Вопрос').Answer('Отвечать').AskMenu('Спросите ' + CPluginName).DockableMenu('Закрепляемый ' + CPluginName).SettingMenu('Настройка ' + CPluginName).BaseUrl('Базовый URL').
...
//Spanish
  ...
  Question('Pregunta').Answer('Respuesta').AskMenu('Pregunte ' + CPluginName).DockableMenu('Acoplable ' + CPluginName).SettingMenu('Configuración de ' + CPluginName).
...
//Urdu
  ...
  Question('سوال').Answer('جواب دیں۔').AskMenu(CPluginName + ' سے پوچھیں۔').DockableMenu(CPluginName + ' ڈاک کے قابل').SettingMenu(CPluginName + ' سیٹنگ').
